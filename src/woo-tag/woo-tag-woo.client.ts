...
export class WooTagWooClient {
	...
	async fetchAllTags(): Promise<any[]> {
		...
		try {
			...
			let page = 1
			while (true) {
				const res = await this.api.get('/products/tags', {
					params: {
						per_page: 100,
						page,
						orderby: 'name',
	...
	async fetchProductsByTag(tagId: number, perPage = 30): Promise<any[]> {
		try {
			const res = await this.api.get('/products', {
				params: { tag: tagId, per_page: perPage, status: 'publish' }
			})
			return res.data || []
		} catch {
			return []
